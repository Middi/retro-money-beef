<?php
/**
 * The template for displaying the footer
 *
 * Contains the closing of the #content div and all content after.
 *
 * @link https://developer.wordpress.org/themes/basics/template-files/#template-partials
 *
 * @package retro_money_beef
 */

?>


<?php wp_footer(); ?>
    </div><!-- wrapper end -->
   
        <footer>
            <div class="container flex">
                <div class="footer-flex footer-1">
                    <h4 class="footer-title">Discover</h4>
                    <ul class="footer-ul">
                        <li class="footer-li"><a href="/blog">Blog</a></li>
                        <li class="footer-li"><a href="/blog">Reviews</a></li>
                        <li class="footer-li"><a href="/blog">Order Free Pack</a></li>
                    </ul>
                </div>
                <div class="footer-flex footer-2"><h4 class="footer-title">Our Company</h4>
                    <ul class="footer-ul">
                        <li class="footer-li"><a href="/blog">Privacy Policy</a></li>
                        <li class="footer-li"><a href="/blog">T&Cs</a></li>
                    </ul>
                </div>
                <div class="footer-flex footer-3"><h4 class="footer-title">Contact</h4>
                    <ul class="footer-ul">
                        <li class="footer-li"><a href="tel:0113123456">0113123456</a></li>
                        <li class="footer-li"><a href="mailto:info@retromoneybeef.com">info@retromoneybeef.com</a></li>
                        <li class="footer-li"><a href="/messenger">Facebook Messenger</a></li>
                    </ul>
                </div>
                <div class="footer-flex footer-4"><h4 class="footer-title">Social</h4>
                <a href="https://facebook.com" class="social-icons"><svg width="28" height="28" xmlns="http://www.w3.org/2000/svg"><g id="Social-Icons" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"><g id="social-icons" transform="translate(-40 -40)" fill="#000"><g id="facebook" transform="translate(40 40)"><path d="M14 0c7.732 0 14 6.268 14 14s-6.268 14-14 14S0 21.732 0 14 6.268 0 14 0zm3.168 7.636h-2.072c-2.488 0-3.363 1.224-3.363 3.282v1.514h-1.551v2.522h1.55v7.319h3.104v-7.32h2.071l.275-2.521h-2.346l.004-1.263c0-.657.064-1.01 1.033-1.01h1.295V7.636z" id="Combined-Shape"/></g></g></g></svg></a>
                <a href="https://instagram.com" class="social-icons"><svg width="28" height="28" xmlns="http://www.w3.org/2000/svg"><g id="Social-Icons" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"><g id="social-icons" transform="translate(-220 -40)" fill="#000"><g id="instagram" transform="translate(220 40)"><path d="M14 0c7.732 0 14 6.268 14 14s-6.268 14-14 14S0 21.732 0 14 6.268 0 14 0zm.885 7h-1.77c-1.12.004-1.39.014-2.001.042-.745.034-1.254.152-1.7.326a3.43 3.43 0 00-1.24.807c-.388.389-.628.78-.807 1.24-.173.445-.29.954-.325 1.699-.028.61-.038.881-.041 2v1.771c.003 1.12.013 1.39.041 2.001.034.745.152 1.254.325 1.7.18.46.419.85.808 1.24.389.388.78.628 1.24.807.445.173.954.29 1.699.325.747.034.985.042 2.886.042 1.901 0 2.14-.008 2.886-.042.745-.034 1.254-.152 1.7-.325.46-.18.85-.419 1.24-.808.388-.389.628-.78.806-1.24.174-.445.292-.954.326-1.699.034-.747.042-.985.042-2.886 0-1.901-.008-2.14-.042-2.886-.034-.745-.152-1.254-.326-1.7a3.431 3.431 0 00-.807-1.24 3.432 3.432 0 00-1.24-.806c-.445-.174-.954-.292-1.699-.326-.61-.028-.881-.038-2-.041zm1.944 1.302c.682.031 1.053.145 1.3.241.326.127.56.279.804.524.245.245.397.478.524.805.096.246.21.617.24 1.3.03.637.04.889.041 2.15v1.356c-.002 1.261-.011 1.513-.04 2.15-.031.683-.145 1.054-.241 1.3-.127.327-.279.56-.524.805a2.169 2.169 0 01-.805.524c-.246.096-.617.21-1.3.241-.637.029-.889.038-2.15.04h-1.356c-1.261-.002-1.513-.011-2.15-.04-.683-.031-1.054-.145-1.3-.241a2.169 2.169 0 01-.805-.524 2.169 2.169 0 01-.524-.805c-.096-.246-.21-.617-.241-1.3-.029-.637-.038-.889-.04-2.15v-1.356c.002-1.261.011-1.513.04-2.15.031-.683.145-1.054.241-1.3.127-.327.279-.56.524-.805s.478-.397.805-.524c.246-.096.617-.21 1.3-.24.637-.03.889-.04 2.15-.041h1.356c1.261.002 1.513.011 2.15.04zm-2.815 2.115a3.597 3.597 0 100 7.194 3.597 3.597 0 000-7.194zm0 1.262a2.335 2.335 0 110 4.67 2.335 2.335 0 010-4.67zm3.708-2.262a.833.833 0 100 1.666.833.833 0 000-1.666z" id="Combined-Shape"/></g></g></g></svg></a>
                <a href="https://youtube.com" class="social-icons"><svg width="28" height="28" xmlns="http://www.w3.org/2000/svg"><g id="Social-Icons" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"><g id="social-icons" transform="translate(-40 -100)" fill="#000"><g id="youtube" transform="translate(40 100)"><path d="M14 0c7.732 0 14 6.268 14 14s-6.268 14-14 14S0 21.732 0 14 6.268 0 14 0zm1.812 12.947c-2.593-.058-6.856.053-7.4.193a1.31 1.31 0 00-.637.37c-.344.344-.549.837-.631 1.519a23.122 23.122 0 00-.011 5.05c.078.693.239 1.144.54 1.515a1.5 1.5 0 00.802.487c.204.042 1.222.099 2.706.152.221.009.806.022 1.3.03 1.03.019 2.925.008 4.007-.022a96.254 96.254 0 002.463-.104l.201-.013c.43-.024.704-.114.96-.317.333-.262.59-.746.699-1.314.103-.537.172-1.494.185-2.58a19.67 19.67 0 00-.069-2.184c-.083-.993-.148-1.316-.352-1.73-.27-.547-.661-.836-1.217-.9-.383-.044-1.704-.104-3.045-.14zm-.688 1.488v1.234c.003.413.01.718.016.718.009 0 .089-.071.179-.158.206-.2.377-.286.595-.303.39-.03.622.17.728.63.027.12.035.256.036 1.088v1.1c-.002 1.069-.017 1.107-.129 1.357-.067.15-.216.297-.34.332-.318.095-.575.019-.87-.26a1.104 1.104 0 00-.191-.158c-.016 0-.024.067-.024.201v.202h-.76v-5.983h.76zm-3.066 1.549l.005 1.808c.006 1.777.007 1.81.05 1.882.055.097.142.123.278.085.08-.022.135-.062.265-.191l.164-.164v-3.42h.761v4.434h-.761v-.312c-.003-.115-.01-.155-.027-.155a.611.611 0 00-.153.128 1.43 1.43 0 01-.482.335c-.315.118-.614.05-.742-.171-.1-.172-.115-.23-.118-1.537v-2.722h.76zm6.127.005c.543-.198 1.197.053 1.433.55.124.263.14.375.152 1.115l.01.663-.81.006-.811.005-.006.445c-.009.629.033.822.202.937.142.096.36.088.48-.016.111-.099.143-.204.154-.512l.01-.27h.783l-.012.301a2.816 2.816 0 01-.045.425 1.094 1.094 0 01-.794.787c-.55.154-1.153-.099-1.394-.582-.12-.243-.137-.312-.14-1.275v-.722c.002-.77.009-.888.039-1.018.097-.419.34-.689.749-.839zm-9.869-1.555l1.39.006 1.39.005.012.795-.481.006-.482.005-.01 2.578-.011 2.578-.419.006c-.322.004-.42 0-.427-.021a160.43 160.43 0 01-.016-2.584l-.005-2.557-.47-.005-.47-.006v-.806zm6.925 2.346l-.074.057v2.706l.074.056c.167.129.351.155.478.07a.285.285 0 00.108-.154c.033-.096.036-.219.031-1.357-.004-1.186-.007-1.256-.045-1.334-.098-.198-.345-.217-.572-.044zm3.342-.168c-.204 0-.333.1-.394.302-.026.087-.053.682-.034.734.01.028.07.033.427.033h.416l-.008-.377c-.007-.345-.011-.385-.057-.479-.072-.147-.18-.213-.35-.213zm-.024-8.718l-.428.005-.428.006-.005 1.724-.006 1.724-.143.146c-.15.153-.269.21-.377.184-.118-.03-.115.016-.125-1.959l-.009-1.819h-.857v2.781c.002 1.31.015 1.337.106 1.517.085.17.21.252.422.28.28.038.582-.09.862-.363l.121-.12.001.283c.002.109.01.13.036.138.053.015.702.016.772.001l.058-.012V7.894zm-3.515.155a1.234 1.234 0 00-1.26-.076c-.304.151-.5.4-.6.767-.043.154-.046.221-.054 1.295-.01 1.386.006 1.55.179 1.865.17.31.522.536.905.582.115.015.35-.008.484-.045a1.2 1.2 0 00.728-.603c.135-.285.141-.362.141-1.68V9.81c-.001-.769-.007-.877-.039-1.014-.082-.35-.234-.585-.484-.747zM12.72 6.364c-.39-.002-.508 0-.519.01-.005.005-.14.517-.378 1.444-.115.442-.21.796-.214.785l-.304-1.122c-.164-.606-.3-1.106-.305-1.11a22.693 22.693 0 00-.95-.005c-.06 0-.094-.119.547 1.927.088.283.221.684.295.89.226.638.28.888.256 1.204-.018.247-.021 1.993-.003 2.023.013.022.829.032.907.01.03-.007.035-.07.036-.892V9.953l.062-.197c.035-.108.12-.392.191-.63l.19-.638a79.56 79.56 0 00.61-2.052l.01-.07zm1.644 2.26l.065.011c.138.038.209.124.25.306.029.134.032.302.026 1.289-.008 1.27-.009 1.279-.156 1.391a.397.397 0 01-.3.058c-.078-.018-.184-.141-.217-.255-.044-.15-.045-2.388-.001-2.539.057-.195.22-.297.398-.25z" id="Combined-Shape"/></g></g></g></svg></a>
                <a href="https://twitter.com" class="social-icons"><svg width="28" height="28" xmlns="http://www.w3.org/2000/svg"><g id="Social-Icons" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"><g id="social-icons" transform="translate(-100 -40)" fill="#000"><g id="twitter" transform="translate(100 40)"><path d="M14 0c7.732 0 14 6.268 14 14s-6.268 14-14 14S0 21.732 0 14 6.268 0 14 0zm2.888 8.91c-1.514 0-2.74 1.294-2.74 2.89 0 .227.022.448.07.659-2.278-.12-4.298-1.27-5.651-3.021a3.006 3.006 0 00-.371 1.454c0 1.003.484 1.888 1.22 2.407a2.644 2.644 0 01-1.243-.361v.036c0 1.4.944 2.57 2.2 2.835a2.63 2.63 0 01-1.238.051c.348 1.147 1.36 1.985 2.56 2.007a5.323 5.323 0 01-4.059 1.198 7.476 7.476 0 004.203 1.299c5.044 0 7.8-4.406 7.8-8.228 0-.125-.001-.25-.007-.374A5.731 5.731 0 0021 10.265c-.492.23-1.02.386-1.574.455a2.87 2.87 0 001.205-1.6 5.317 5.317 0 01-1.742.702 2.669 2.669 0 00-2-.913z" id="Combined-Shape"/></g></g></g></svg></a>
                </div>
            </div>
            <p class="copyright text-center">
                Copyright Retro Money Beef 2020
            </p>
        </footer>
    </div> <!-- page end -->
</body>

</html>